...
public:
  mean(MixtureModel* mix, const int idx, const int k, const double min,
       const double max)
    : Parameter("mean"), mix_(mix), idx_(idx)
  {
    Util::Assert<MixtureAssert>(max > min);
    // fraction of distance from min to max
    double p = static_cast<double>(idx)/(k-1);
    double x = min + p*(max - min);
    Assign(x);
...
public:
  component(MixtureModel* mix, const int idx, const int nComp,
...
public:
  MixtureModel(const int nBurnin, const int nSample, const int thin,
               vector<double>& x, const int nComponents,
               const double minMu, const double maxMu, 
               const double var, std::string file)
    : Model(nBurnin, nSample, thin), x_(x), nComp_(nComponents), 
      nElem_(x_.size()), logFile_(file)
  {
    using Util::Assert;
    // means
    for (int i = 0; i < nComp_; ++i) {
      step_.push_back(new MuStep(new mean(this, i, nComp_, minMu, maxMu), 
                                 this, i));
    }
    // variances
    varOffset_ = nComp_;
    ...
    for (int i = 0; i < nComp_; ++i) {
      // if all components are in equal frequency and have equal variance,
      // total variance = n*(component variance)
...
private:
  ...
  void write(const SampleVector& p) {
    using std::ios;
    std::ofstream outf(logFile_.c_str(), ios::app);
...
double
mean::llike(const double mu0) const {
  int nElem = mix_->NElem();
  double llike = 0.0;
  for (int i = 0; i < nElem; ++i) {
    int idx = mix_->Idx(i);
    // only components whose index matches the mean index being updated
...
mean::lPrior(const double mu) const {
  double lprior;
  if (idx_ == 0) {
    lprior = (mu > mix_->Mu(1)) ? Util::log_dbl_min : 
      Density::dnorm(mu, 0.0, sqrt(1000.0), true);
  } else {
    lprior = (mu < mix_->Mu(idx_-1)) ? Util::log_dbl_min : 
      Density::dnorm(mu, mix_->Mu(idx_-1), sqrt(1000.0), true);
  }
  return lprior;
}

double
variance::llike(const double var0) const {
  int nElem = mix_->NElem();
  double llike = 0.0;
  for (int i = 0; i < nElem; ++i) {
    int idx = mix_->Idx(i);
    // only components whose index matches the variance index being updated
    // contribute to the likelihood
    if (idx_ == idx) {
...
p::Function(const bool doCalc) const {
  double k = mix_->Lambda(idx_);
  double n = mix_->CumLambda();
  return k/n;
}

double
component::llike(const int k) const {
  return Density::dnorm(mix_->X(idx_), mix_->Mu(k), mix_->Sd(k), true);
}

double
component::lPrior(const int k) const {
  return log(mix_->P(k));
}

int
component::propose(int k) const {
  int newK = static_cast<int>(floor(rng_.uniform()*mix_->K()));
...
int main(int ac, char** av) {
  ...
  int nComponents;
  parse(av[2], int_p[assign(nComponents)], space_p);
  ...
  std::string logFile = "norm-mixture-log.txt";
  double minMu = Util::vectorMin(x);
  double maxMu = Util::vectorMax(x);
  SimpleStatistic xStat(x);
  MixtureModel model(nBurnin, nSample, thin, x, nComponents, minMu, maxMu, 
                     xStat.Variance(), logFile.c_str());
  std::ofstream outf(logFile.c_str(), ios::out);
  ...
  Util::Assert<MixtureAssert>(!argCheck_ || false);
}
